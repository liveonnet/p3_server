...
class Tools(object):
    ...
    def get_major_version(ver, sep='.'):
        ...
        '''
        arr_ver = filter(None, ver.split(sep))  # '1..1.' == '1.1'
...
#-#            info('birth_day: %s', birth_day)
    ...
    def checkDeviceId(device_id):
        u''' 判断device_id(android的device_id，ios的idfa)是否规范
...
def dict2Xml(d_data):
    u'''基于字典对象 ``d_data`` 构造xml，不支持多级元素，不支持属性

    以key的字典序构造元素
    '''
    s = StringIO()
    s.write('<xml>')
    for _k, _v in sorted(d_data.iteritems()):
        if _v:
...
def str2_int_list(s, sep=','):
    return map(int, filter(None, s.split(sep)))
...
def str2_str_list(s, sep=','):
    u'''将字符串 ``s`` 按分隔符 ``sep`` 分隔为列表，不保留空值
    '''
    return filter(None, (x.strip() for x in s.split(sep)))
